...
%%% STOP
En plus de cela les processus d'execution sont unique à chaque utilisateur mais ce dernier s'exécute dans un envirronement partagé. L'ouverture aux utilisateurs de l'éditeur de code au sein d'HappyBox n'a pas été ouverte pour des raisons de sécurité afin d'éviter une exécution de code malveillant sur nos serveurs.

\subsection{Outils de travail}
Le code et la technique ne font pas tout au seins d'une startup. Il nécessaire de pouvoir élaborer un stratégie et d'en controller l'efficacité. En ce sens la mise en place de git à profondément changé les méthodes de travail en les rendant plus collaboratives et plus sécuritaire. Gitlab est certe un logiciel de gestion de version des sources mais c'est aussi une plateforme social facilitant la communication. 

Dans le même soucis d'amélioration des conditions de travail le choix de la plateforme Jira nous à permis d'organiser l'équipe. Chaque membre peut facilement savoir ce qui lui est necessaire de faire pour avancé de manière coordonner avec le reste de l'équipe. La communication s'en trouve une fois de plus simplifié. 

Jira à aussi profondément changé les méthodes de travail en créant un processus de dévellopement et de gestion de la stratégie claire. Le système de ticket trié par importance permet de savoir rapidement ce qu'il y à faire et ce qui à été fait. Il aussi possible de générer des indiquateur de performance et définir des métriques telles que le nombre de ticket résolu par jours ou les objectifs restant avant la prochaine version. 

Avant cela il n'y avait pas vraiment de processus de management au vue de la taille de l'équipe. L'utilisation d'un logiciel SaaS permet de le rendre accessible depuis n'importe quelle point. C'est un aspect important pour happybox car dans soucis de liberté il n'y a aucune obligation de venir au bureau physiquement. Nous sommes la pour faire avancer un projet et faire un quota d'heure. Que le travail soit fait du bureau ou depuis son canapé un employé heureux est un employé efficace. 
...
% resultat
Pour le moment les résultats ne sont que peu visible 
...
Il est contradictoire d'empêcher l'expression créative de nos clients alors que nous voulons justement que notre plateforme soit utilisée. Pourquoi donnons-nous une multitude de fonctionnalités à leur disposition sans leur permettre de pouvoir tous les essayer et ce dans des projets différents ? Bien sûr, il est toujours facile de se rendre compte de ces choses après coup. 

J'ai pris conscience de mon manque de discernement et surtout du peu d'expérience que j'avais lors de mon arrivée au sein d'HappyBox. Aujourd'hui, j'ose entrapercevoir les problèmes qu'HappyBox a pu rencontré dans leur communication interne et externe de leur produit. 
