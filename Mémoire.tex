...
% Best practice linux bible
	...
	\section{YarrBox\footnote{"Yarr" est le cris d'un pirate. Il faut prononcé ce nom en fermant un oeil et en grognant.} le seul est unique vaisseaux des pirates de la boite.} % 5 page
...
En ce sens c'est un outils exeptionnel top souvent ignoré car son interface graphique n'est pas trés agréable. Mais dasn notre l'interface n'est pas utilisé. ces données permettent de fournir des statistiques d'utilisation qui sont accessible pour les utilisateurs HappyBox. Danny à interfacé le résultat de mes scripts à l'application.
%script de manipulation LVM

\subsection{stockage}
En ce qui concerne le stockage des données utilisateurs. Il y a eu beaucoup de progrés pour les libérer des instances et rendre encore plus flexible HappyBox. Malgrés cela la solution n'est pas encore au point à mon avis. Tant que les données utilisateurs seront lié à une seul et unique instance leurs gestion sera complexe et chronophage. 

C'est pourquoi une nouvelle approche doit être envisagé. Jusqu'à l'heure peu de solutions semblé répondre convenablement au problème  et s'interfacer judicieusement sans créer un énorme usine à gaz. 

Ceci n'est plus vrai depuis la sortie du logiciel Docker. Il permet de cloisonné ou plusieurs processus dans des conteneurs ou il s'execute sans affecter l'hote. 

En effet il serait ,grace à lui, possible de séparer les espaces des utilisateurs dans des conteneur autonome et fortement cloisoné. De la sorte au lieu d'offrir non seulement un hébergement pour les sites des utilisateurs mais aussi une machine virtuelle ou il pourrait installer les logiciels de leurs choix et serait \emph{maitre} d'un petit espace sur les serveurs de HappyBox. Cela correspondrai à l'ouverture de la version agence du logiciel à tout les utilisateurs. 
